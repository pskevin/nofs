...
void log_msg(const char *format, ...)
{
    ...
    vfprintf(NOFS_DATA->logfile, format, ap);
}
...
void log_fuse_context(struct fuse_context *context)
{
    ...
    log_struct(((struct nofs_state *)context->private_data), logfile, %08x, );
    log_struct(((struct nofs_state *)context->private_data), root_dir, %s, );
